...
class QuestionCreate(BaseModel):
    ...
    category: str = Field(
        pattern="^(common|job|foreigner)$", description="카테고리 (common: 공통, job: 직무별, foreigner: 외국인특화)"
    )
    job_type: JobType | None = Field(default=None, description="직무 타입 (category가 job인 경우 권장, it/marketing)")
    level: Level | None = Field(default=None, description="경력 레벨 (intern: 인턴, entry: 신입, experienced: 경력)")
...
class InterviewSetCreate(BaseModel):
    ...
    question_count: int = Field(default=3, ge=1, le=10, description="질문 개수 (1~10개, 기본값 3개)")
...
class InterviewSetResponse(BaseModel):
    ...
    status: InterviewSetStatus = Field(
        description="면접 상태: in_progress(면접중), pending_evaluation(평가대기), completed(평가완료)"
    )
    created_at: datetime = Field(description="생성 일시")
...
class SubmitFollowUpRequest(BaseModel):
    ...
    follow_up_answer: str | None = Field(default=None, min_length=1, description="꼬리질문에 대한 답변 (텍스트)")
    audio: AudioInput | None = Field(default=None, description="음성 입력 (아직 미지원)")
...
class InterviewSetDetailResponse(BaseModel):
    ...
    questions: list[QuestionInfo] = Field(
        default_factory=list, description="면접 세트에 배정된 질문 목록 (이어하기 시 이 목록 사용)"
    )
...
class AnswerNoteEntryCreate(BaseModel):
    """답변 노트 항목 생성 요청
...
class AnswerNoteEntryUpdate(BaseModel):
    """답변 노트 항목 수정 요청
...
class AnswerNoteEntryResponse(BaseModel):
    """답변 노트 항목 응답"""
...
class AnswerNoteCreate(BaseModel):
    ...
    여러 질문 답변을 묶어 저장하는 노트를 생성합니다.
    """

    title: str = Field(min_length=1, max_length=100, description="답변 노트 제목")
    entries: list[AnswerNoteEntryCreate] | None = Field(
        default=None, description="초기 생성 시 함께 추가할 답변 항목 목록 (선택)"
    )
...
class AnswerNoteUpdate(BaseModel):
    ...
    title: str | None = Field(default=None, min_length=1, max_length=100, description="답변 노트 제목")


class AnswerNoteSummaryResponse(BaseModel):
    """답변 노트 요약 응답 (목록용)"""

    model_config = ConfigDict(from_attributes=True)

    id: UUID = Field(description="답변 노트 고유 ID")
    title: str = Field(description="답변 노트 제목")
    entries_count: int = Field(description="노트에 포함된 답변 항목 수")
    created_at: datetime = Field(description="생성 일시")
    updated_at: datetime = Field(description="마지막 수정 일시")


class AnswerNoteDetailResponse(BaseModel):
    """답변 노트 상세 응답"""
    ...
    created_at: datetime = Field(description="생성 일시")
    updated_at: datetime = Field(description="마지막 수정 일시")
    entries: list[AnswerNoteEntryResponse] = Field(description="노트에 포함된 답변 항목 목록")
